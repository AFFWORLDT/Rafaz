...
export default function DetailPage({ id }: { id: string | string[] | undefined }) {
  ...
  const fetchPropertyDetails = async () => {
    ...
    try {
      ...
      if (!idParam) {
        console.error('Buy Detail - No ID provided');
        setLoading(false);
        return;
      }
      
      console.log('Buy Detail - Fetching property with slug/ID:', idParam);
      
      // Check if it's a numeric ID - if so, fetch by ID first, then redirect to name-based URL
      if (/^\d+$/.test(idParam)) {
        console.log('Buy Detail - Numeric ID detected, fetching property by ID first:', idParam);
        try {
          ...
          if (propertyData) {
            ...
            if (propertyName) {
              ...
              console.log('Buy Detail - Redirecting to name-based URL:', `/buy/details/${slug}`);
              window.location.href = `/buy/details/${slug}`;
              return;
            }
          }
        } catch (error) {
          console.error('Buy Detail - Error fetching property by ID:', error);
        }
        
        console.error('Buy Detail - Could not fetch property by ID or generate name-based URL');
        setLoading(false);
        return;
      }
      ...
      console.log('Buy Detail - API Response:', res);
      
      // Get the exact property from response
      const propertyData = res?.properties?.[0];
      
      if (!propertyData) {
        console.error('Buy Detail - Property not found for slug:', idParam);
        setLoading(false);
        return;
      }
      ...
      console.log('Buy Detail - Property name matches slug exactly:', propertyName);
      console.log('Buy Detail - Property data:', propertyData);
